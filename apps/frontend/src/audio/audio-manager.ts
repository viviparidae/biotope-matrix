import { SimulationEvent } from '../../../../packages/shared-types/src/ecs';
...
export class AudioManager {
  ...
  handleEvent(event: SimulationEvent): void {
    ...
    if (event.type === 'terrain-change') this.playTone(event.terrain === 2 ? 150 : 90, 0.14, 'triangle', event.x);
    if (event.type === 'breeding-mode') this.playTone(660, 0.24, 'sine', event.x);
    if (event.type === 'meteor') this.playTone(55, 0.8, 'sawtooth', event.x);
    if (event.type === 'flood') this.playTone(120, 0.6, 'triangle', event.x);
  }
